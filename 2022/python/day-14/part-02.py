...
input = open("./2022/python/day-14/input-sample.txt")
...
def add_one_sand():
    ...
    while (can_sand_drop):
        if sand.y > last_rock_formation:
            ...
            sand.x += 1
            sand.print_coordinate()
        elif compare_points(sand, sand_origin) == "equal":
            can_sand_drop = False
            status = "can no longer add sand"
            sand.print_coordinate()
        else:
...
last_rock_formation = get_last_rock_formation() 

# add floor
grid[last_rock_formation + 2 ] = [rock_symbol for i in range(grid_size)]
last_rock_formation = last_rock_formation + 2 

print_grid(500, 75, last_rock_formation + 3)        

# start adding sand
counter = 0
while 1 == 1:
    if add_one_sand() in ["sand falls into abyss", "can no longer add sand"]:
        break
    counter += 1

print_grid(500, 125, last_rock_formation + 3)

counter += 1 # add_one_sand() does not overwrite the origin point so add one

logging.info(f"Using your scan, simulate the falling sand until the source of the sand becomes blocked. How many units of sand come to rest? {counter}")