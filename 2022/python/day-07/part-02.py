...
root_total = sum([directory["total_size"] for directory in directory_totals if directory["directory_path"] == "/"])

print(f"total used space: {root_total}")
unused_space = 70000000 - root_total
print(f"unused space: {unused_space}")
space_needed = 30000000 - unused_space
print(f"space needed: {space_needed}")

current_lowest = root_total
for directory in directory_totals:
    # if directory['total_size'] >= space_needed and directory['total_size'] < current_lowest:
    if space_needed <= directory['total_size'] < current_lowest:
        current_lowest = directory['total_size']

print(f'Find the smallest directory that, if deleted, would free up enough space on the filesystem to run the update. What is the total size of that directory? {current_lowest}')